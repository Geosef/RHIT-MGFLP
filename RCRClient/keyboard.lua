...
local registeredTextFields = {}

function Keyboard:registerTextField(tf, outerSprite)
	local clickable
	if outerSprite then clickable = outerSprite
	else clickable = tf end
	
	function click(event)
		if clickable:hitTestPoint(event.touches[1].x, event.touches[1].y) then 
			if not this.showing then
				stage:addChild(this)
				textField = tf
				this:show(function()
					this.showing = false
	...
	clickable:addEventListener(Event.TOUCHES_BEGIN, clickable.click)
	table.insert(registeredTextFields, clickable)
end

function Keyboard:unregisterTextFields()
	for i,tf in ipairs(registeredTextFields) do
		tf:removeEventListener(Event.TOUCHES_BEGIN, tf.click)
	end
